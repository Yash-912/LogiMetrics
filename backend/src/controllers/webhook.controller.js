...
const { WebhookEndpoint, WebhookDelivery, Company, Shipment, Payment } = require('../models/postgres');
...
const getWebhookEndpoints = async (req, res, next) => {
  try {
    ...
    return success(res, 'Webhook endpoints retrieved', 200, { endpoints });
  } catch (err) {
    next(err);
  }
};
...
const getWebhookEndpointById = async (req, res, next) => {
  try {
    ...
    // Authorization check
    if (endpoint.companyId !== req.user.companyId && req.user.role !== 'admin') {
      throw new AppError('Not authorized', 403);
    }

    return success(res, 'Webhook endpoint retrieved', 200, { endpoint });
  } catch (err) {
    next(err);
  }
};
...
const createWebhookEndpoint = async (req, res, next) => {
  try {
    ...
    logger.info('Webhook endpoint created', { endpointId: endpoint.id, userId: req.user.id });

    return success(res, 'Webhook endpoint created successfully', 201, {
      endpoint: {
        ...endpoint.toJSON(),
...
const updateWebhookEndpoint = async (req, res, next) => {
  try {
    ...
    return success(res, 'Webhook endpoint updated successfully', 200, { endpoint });
  } catch (err) {
    next(err);
  }
};
...
const deleteWebhookEndpoint = async (req, res, next) => {
  try {
    ...
    return success(res, 'Webhook endpoint deleted successfully', 200);
  } catch (err) {
    next(err);
  }
};
...
const rotateSigningSecret = async (req, res, next) => {
  try {
    ...
    return success(res, 'Signing secret rotated successfully', 200, {
...
const testWebhookEndpoint = async (req, res, next) => {
  try {
    ...
    return success(res, 'Test webhook sent', 200, {
...
const getWebhookDeliveries = async (req, res, next) => {
  try {
    ...
    const { rows: deliveries, count } = await WebhookDelivery.findAndCountAll({
      where,
      include: [
        { model: WebhookEndpoint, as: 'endpoint', attributes: ['id', 'name', 'url'] }
      ],
      order: [['createdAt', 'DESC']],
    ...
    return success(res, 'Webhook deliveries retrieved', 200, {
      deliveries,
      pagination: {
        total: count,
...
const retryWebhookDelivery = async (req, res, next) => {
  try {
    ...
    // Authorization check
    if (delivery.endpoint.companyId !== req.user.companyId && req.user.role !== 'admin') {
      throw new AppError('Not authorized', 403);
    }
    ...
    return success(res, result.success ? 'Webhook delivered successfully' : 'Webhook delivery failed', 200, {
      success: result.success,
      statusCode: result.statusCode,
      attempts: delivery.attempts + 1
    });
    ...
    const events = [
      {
        category: 'shipment',
        events: [
          { name: 'shipment.created', description: 'Triggered when a new shipment is created' },
          { name: 'shipment.updated', description: 'Triggered when a shipment is updated' },
          { name: 'shipment.status_changed', description: 'Triggered when shipment status changes' },
          { name: 'shipment.picked_up', description: 'Triggered when shipment is picked up' },
          { name: 'shipment.in_transit', description: 'Triggered when shipment is in transit' },
          { name: 'shipment.out_for_delivery', description: 'Triggered when shipment is out for delivery' },
          { name: 'shipment.delivered', description: 'Triggered when shipment is delivered' },
          { name: 'shipment.cancelled', description: 'Triggered when shipment is cancelled' },
          { name: 'shipment.exception', description: 'Triggered when shipment has an exception' }
        ]
      },
      {
        category: 'tracking',
        events: [
          { name: 'tracking.location_update', description: 'Triggered on location update' },
          { name: 'tracking.geofence_enter', description: 'Triggered when entering a geofence' },
          { name: 'tracking.geofence_exit', description: 'Triggered when exiting a geofence' },
          { name: 'tracking.eta_changed', description: 'Triggered when ETA changes significantly' }
        ]
      },
      {
        category: 'invoice',
        events: [
          { name: 'invoice.created', description: 'Triggered when an invoice is created' },
          { name: 'invoice.sent', description: 'Triggered when an invoice is sent' },
          { name: 'invoice.paid', description: 'Triggered when an invoice is paid' },
          { name: 'invoice.overdue', description: 'Triggered when an invoice becomes overdue' }
        ]
      },
      {
        category: 'payment',
        events: [
          { name: 'payment.succeeded', description: 'Triggered when a payment succeeds' },
          { name: 'payment.failed', description: 'Triggered when a payment fails' },
          { name: 'payment.refunded', description: 'Triggered when a payment is refunded' }
        ]
      },
      {
        category: 'driver',
        events: [
          { name: 'driver.status_changed', description: 'Triggered when driver status changes' },
          { name: 'driver.assigned', description: 'Triggered when driver is assigned to shipment' }
        ]
      },
      {
        category: 'vehicle',
        events: [
          { name: 'vehicle.status_changed', description: 'Triggered when vehicle status changes' },
          { name: 'vehicle.maintenance_due', description: 'Triggered when vehicle maintenance is due' }
        ]
      }
    ];

    return success(res, 'Available events retrieved', 200, { events });
  } catch (err) {
    next(err);
  }
};
    ...
    if (MessageStatus) {
      ...
      await Notification.update(
        { 
          ...
          smsDeliveredAt: MessageStatus === 'delivered' ? new Date() : null
        },
...
const deliverWebhook = async (endpoint, payload, isTest = false, existingDeliveryId = null) => {
    ...
    if (!existingDeliveryId && !isTest) {
      await WebhookDelivery.create({
        ...
        lastResponse: {
          ...
          body: typeof response.data === 'object' ? response.data : response.data?.substring(0, 1000)
        },
        responseTime
      });
    }
