...
const { Shipment, Vehicle, Driver } = require("../models/mongodb");
...
async function calculateDashboardMetrics(companyId) {
  ...
  return {
    ...
    vehicles: {
      total: vehicleStats.reduce(
        (sum, v) => sum + parseInt(v.dataValues.count),
        0
      ),
    ...
    drivers: {
      total: driverStats.reduce(
        (sum, d) => sum + parseInt(d.dataValues.count),
        0
      ),
...
async function getShipmentSummary(companyId) {
  ...
  const [pending, inTransit, outForDelivery, delivered, delayed] =
    await Promise.all([
