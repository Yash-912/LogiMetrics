...
const { Vehicle, Driver, User, Company } = require("../models/mongodb");
...
async function checkVehicleMaintenance() {
  ...
  try {
    ...
    const noticeDate = new Date(
      Date.now() + ALERT_THRESHOLDS.NOTICE * 24 * 60 * 60 * 1000
    );
    ...
    logger.info(
      `[MaintenanceJob] Found ${vehiclesNeedingMaintenance.length} vehicles needing maintenance attention`
    );
      ...
      try {
        ...
        if (nextServiceDate) {
          daysUntilDue = Math.ceil(
            (new Date(nextServiceDate) - today) / (1000 * 60 * 60 * 24)
          );
        ...
        const odometerMessage =
          nextServiceOdometer && vehicle.odometer >= nextServiceOdometer
        ...
        const message = `Vehicle ${vehicle.registrationNumber} (${
          vehicle.make
        } ${vehicle.model}) ${
          daysUntilDue <= 0
        ...
        for (const user of recipients) {
          await notificationService.sendNotification(user.id, {
            ...
            message,
            channels:
              alertLevel === "urgent" ? ["in_app", "email"] : ["in_app"],
        ...
        logger.error(
          `[MaintenanceJob] Failed to send maintenance alert for vehicle ${vehicle.id}:`,
          error
        );
      }
    }
    ...
    logger.info(
      `[MaintenanceJob] Sent ${alertsSent} vehicle maintenance alerts`
    );
  } catch (error) {
...
async function checkLicenseExpiry() {
  ...
  try {
    ...
    logger.info(
      `[MaintenanceJob] Found ${driversWithExpiringLicense.length} drivers with expiring licenses`
    );
      ...
      try {
        ...
        if (driver.user) {
          await notificationService.sendNotification(driver.user.id, {
            ...
            message,
            channels:
              alertLevel === "expired" || alertLevel === "urgent"
                ? ["in_app", "email", "sms"]
                : ["in_app"],
            data: {
              driverId: driver.id,
        ...
        if (alertLevel === "expired" && driver.status !== "inactive") {
          ...
          logger.warn(
            `[MaintenanceJob] Deactivated driver ${driver.id} due to expired license`
          );
        }
        ...
        logger.error(
          `[MaintenanceJob] Failed to send license expiry alert for driver ${driver.id}:`,
          error
        );
      }
    }
    ...
    for (const vehicle of vehicles) {
      try {
        ...
        for (const warning of warnings) {
          ...
          const alertLevel =
            warning.daysUntilExpiry <= ALERT_THRESHOLDS.URGENT
              ? "urgent"
              : "warning";
        ...
        logger.error(
          `[MaintenanceJob] Failed to check documents for vehicle ${vehicle.id}:`,
          error
        );
      }
    }
