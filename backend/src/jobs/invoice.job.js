...
const { Invoice, Company, User } = require("../models/mongodb");
...
async function processRecurringInvoices() {
  ...
  try {
    ...
    logger.info(
      `[InvoiceJob] Found ${recurringInvoices.length} recurring invoices to process`
    );
    ...
    for (const templateInvoice of recurringInvoices) {
      try {
        ...
        successCount++;
        logger.info(
          `[InvoiceJob] Created recurring invoice: ${newInvoice.invoiceNumber}`
        );
      } catch (error) {
        errorCount++;
        logger.error(
          `[InvoiceJob] Failed to process recurring invoice ${templateInvoice.id}:`,
          error
        );
      }
    }
    ...
    logger.info(
      `[InvoiceJob] Recurring invoices processed: ${successCount} success, ${errorCount} errors`
    );
  } catch (error) {
...
async function sendPaymentReminders() {
  ...
  try {
    ...
    logger.info(
      `[InvoiceJob] Found ${invoicesToRemind.length} invoices needing reminders`
    );
    ...
    for (const invoice of invoicesToRemind) {
      try {
        ...
        errorCount++;
        logger.error(
          `[InvoiceJob] Failed to send reminder for invoice ${invoice.id}:`,
          error
        );
      }
    }
    ...
    logger.info(
      `[InvoiceJob] Payment reminders sent: ${sentCount} success, ${errorCount} errors`
    );
  } catch (error) {
...
async function updateOverdueStatus() {
  ...
  try {
    ...
    logger.info(
      `[InvoiceJob] Updated ${updatedCount} invoices to overdue status`
    );
    ...
    if (updatedCount > 0) {
      ...
      for (const invoice of overdueInvoices) {
        try {
          ...
          logger.error(
            `[InvoiceJob] Failed to notify for overdue invoice ${invoice.id}:`,
            error
          );
        }
      }
