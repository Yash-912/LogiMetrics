...
const { Transaction, Invoice, Company, Shipment } = require('../models/postgres');
        ...
        for (const company of companies) {
            ...
            for (const [integrationName, config] of Object.entries(integrations)) {
                ...
                try {
                    ...
                    logger.error(`[SyncJob] Failed to sync ${integrationName} for company ${company.id}:`, error);
                }
            }
        }

        logger.info(`[SyncJob] External sync complete. Success: ${results.success.length}, Errors: ${results.errors.length}`);
    } catch (error) {
...
async function reconcilePayments() {
    ...
    try {
        ...
        logger.info(`[SyncJob] Found ${pendingTransactions.length} pending transactions to reconcile`);
        ...
        for (const transaction of pendingTransactions) {
            try {
                ...
                if (gatewayStatus.status !== transaction.status) {
                    ...
                    reconciledCount++;
                    logger.debug(`[SyncJob] Reconciled transaction ${transaction.id}: ${gatewayStatus.status}`);
                }
            } catch (error) {
                errorCount++;
                logger.error(`[SyncJob] Failed to reconcile transaction ${transaction.id}:`, error);
            }
        }
        ...
        logger.info(`[SyncJob] Payment reconciliation complete. Reconciled: ${reconciledCount}, Errors: ${errorCount}`);
    } catch (error) {
...
async function checkPaymentGatewayStatus(transaction) {
    ...
    logger.debug(`[SyncJob] Checking gateway status for transaction ${transaction.id}`);
...
async function checkMissedWebhooks() {
    ...
    if (oldPendingTransactions.length > 0) {
        logger.warn(`[SyncJob] Found ${oldPendingTransactions.length} transactions without webhook response`);
    }
}
...
async function syncMLPredictions() {
    ...
    try {
        ...
        try {
            ...
            const predictions = await callMLService('/predict/eta', { shipments: shipmentData });
            ...
            logger.info(`[SyncJob] Updated ML predictions for ${predictions?.results?.length || 0} shipments`);
        } catch (mlError) {
    ...
    try {
        ...
        try {
            ...
            if (mongoose.connection.readyState === 1) {
                ...
                health.issues.push({ service: 'mongodb', error: 'Connection not ready' });
            }
        } catch (error) {
            health.services.mongodb = 'unhealthy';
