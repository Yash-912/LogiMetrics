  ...
  try {
    ...
    for (const { model, name } of paranoiModels) {
      try {
        ...
        if (deleted > 0) {
          logger.info(
            `[CleanupJob] Permanently deleted ${deleted} ${name} records`
          );
    ...
    try {
      ...
      if (auditResult.deletedCount > 0) {
        logger.info(
          `[CleanupJob] Deleted ${auditResult.deletedCount} old audit logs`
        );
    ...
    logger.info(
      `[CleanupJob] Database cleanup complete. Total deleted: ${totalDeleted}`
    );
  } catch (error) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        logger.error(
          `[CleanupJob] Error processing log file ${file}:`,
          fileError
        );
      }
    }
    ...
    logger.info(
      `[CleanupJob] Log rotation complete. Archived: ${archivedCount}, Deleted: ${deletedCount}`
    );
  } catch (error) {
...
async function cleanupTempFiles() {
  ...
  try {
    ...
    logger.info(
      `[CleanupJob] Temp cleanup complete. Deleted: ${deletedCount} files, Freed: ${freedMB} MB`
    );
  } catch (error) {
...
async function cleanupExpiredTokens() {
  ...
  try {
    ...
    logger.info(
      `[CleanupJob] Token cleanup complete. Redis: ${deletedFromRedis}, DB: ${updatedCount}`
    );
  } catch (error) {
