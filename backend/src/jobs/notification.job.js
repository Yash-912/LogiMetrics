...
async function processNotificationQueue() {
    ...
    try {
        ...
        if (!redis || !redis.isOpen) {
            logger.warn('[NotificationJob] Redis not available, skipping queue processing');
            return;
        }
        ...
        for (let i = 0; i < batchSize; i++) {
            ...
            try {
                ...
                logger.error('[NotificationJob] Failed to parse notification:', parseError);
            }
        }
        ...
        logger.info(`[NotificationJob] Processing ${notifications.length} notifications`);
        ...
        for (const notification of notifications) {
            try {
                ...
                errorCount++;
                logger.error('[NotificationJob] Failed to process notification:', error);

                // Re-queue failed notifications with retry count
                const retryCount = (notification.retryCount || 0) + 1;
                if (retryCount < 3) {
                    await redis.rPush(NOTIFICATION_QUEUE, JSON.stringify({
                        ...notification,
                        retryCount
                    }));
                }
        ...
        logger.info(`[NotificationJob] Queue processed: ${successCount} success, ${errorCount} errors`);
    } catch (error) {
        logger.error('[NotificationJob] Error processing notification queue:', error);
        throw error;
    }
}
    ...
    await sendEmail({
        ...
        html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px;">
                ...
                <p>${message}</p>
                ${data.actionUrl ? `<p><a href="${data.actionUrl}" style="background: #2563eb; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">View Details</a></p>` : ''}
            </div>
        `
    });
}
...
async function sendSMSNotification(userId, title, message) {
    ...
    logger.debug(`[NotificationJob] SMS would be sent to ${user.phone}: ${title}`);
}
...
async function cleanupOldNotifications() {
    ...
    try {
        ...
        logger.info(`[NotificationJob] Cleaned up ${deletedCount} old notifications`);
    } catch (error) {
        logger.error('[NotificationJob] Error cleaning up old notifications:', error);
        throw error;
    }
}
    ...
    try {
        ...
        for (const user of usersWithUnread) {
            try {
                ...
                await sendEmail({
                    ...
                    html: `
                        <div style="font-family: Arial, sans-serif; max-width: 600px;">
                            ...
                            <p><a href="${process.env.FRONTEND_URL}/notifications" style="background: #2563eb; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">View All Notifications</a></p>
                        </div>
                    `
                });
                ...
                errorCount++;
                logger.error(`[NotificationJob] Failed to send digest to user ${user.id}:`, error);
            }
        }

        logger.info(`[NotificationJob] Digest notifications sent: ${sentCount} success, ${errorCount} errors`);
    } catch (error) {
        logger.error('[NotificationJob] Error sending digest notifications:', error);
        throw error;
    }
}
