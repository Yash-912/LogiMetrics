...
async function handleFailedLogin(user) {
  ...
  if (failedAttempts >= maxAttempts) {
    ...
    logger.warn(
      `Account locked for user ${user.id} after ${failedAttempts} failed attempts`
    );
  }
...
async function storeRefreshToken(userId, token, userAgent, ipAddress) {
  const key = `${REFRESH_TOKEN_PREFIX}${userId}:${crypto
    .createHash("md5")
    .update(token)
    .digest("hex")}`;
...
async function blacklistToken(token) {
  ...
  if (expiresIn > 0) {
    const key = `${TOKEN_BLACKLIST_PREFIX}${crypto
      .createHash("md5")
      .update(token)
      .digest("hex")}`;
