 ...
 */
async function generateFromShipments(
  shipmentIds,
  customerId,
  companyId,
  userId,
  ...
  logger.info(
    `Invoice generated from ${shipments.length} shipments: ${invoice.invoiceNumber}`
  );
  return invoice;
}
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px;">
        ...
        <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
          <p><strong>Amount Due:</strong> ${invoice.currency} ${
      invoice.totalAmount
    }</p>
...
async function createRecurringInvoice(templateInvoiceId, userId) {
  ...
  logger.info(
    `Recurring invoice created: ${invoiceNumber} from template ${templateInvoice.invoiceNumber}`
  );
  return newInvoice;
}
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px;">
        ...
        <p>This is a reminder that invoice ${invoice.invoiceNumber} is ${
...
async function getInvoiceStats(companyId, startDate, endDate) {
  ...
  const [totalInvoiced, paidAmount, pendingAmount, overdueCount] =
    await Promise.all([
