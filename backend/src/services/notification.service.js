...
async function sendShipmentNotification(shipment, status, recipientUserId) {
  ...
  const title = `Shipment ${shipment.trackingNumber} - ${status
    .replace(/_/g, " ")
    .toUpperCase()}`;
  const message = `Your shipment ${
    shipment.trackingNumber
  } status has been updated to: ${status.replace(/_/g, " ")}`;
  ...
  return sendNotification(recipientUserId, {
    ...
    smsMessage: smsTemplates[
      `shipment${
        status.charAt(0).toUpperCase() +
        status.slice(1).replace(/_./g, (m) => m[1].toUpperCase())
      }`
    ]?.(shipment.trackingNumber),
  });
}
