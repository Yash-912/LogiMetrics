...
const { PricingRule, Company, Shipment } = require("../models/mongodb");
...
function checkSurchargeCondition(rule, options) {
  ...
  if (conditions.minWeight && (options.weight || 0) < conditions.minWeight)
    return false;
  if (conditions.maxWeight && (options.weight || 0) > conditions.maxWeight)
    return false;
  if (conditions.vehicleType && options.vehicleType !== conditions.vehicleType)
    return false;
...
async function generateQuote(options) {
  ...
  const quote = {
    quoteId: `QT${Date.now()}${Math.random()
      .toString(36)
      .substr(2, 6)
      .toUpperCase()}`,
    ...rate,
...
async function bulkUpdateRules(companyId, updates) {
  ...
  logger.info(
    `Bulk pricing update: ${successful}/${updates.length} successful`
  );
