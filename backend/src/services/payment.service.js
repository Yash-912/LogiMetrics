...
const { Transaction, Invoice, Company, User } = require("../models/mongodb");
...
async function processPayment(paymentData, processedBy) {
  ...
  try {
    if (paymentGateway === "razorpay") {
      ...
      gatewayResponse = await createRazorpayOrder(
        amount,
        currency,
        transaction.id
      );
...
async function verifyPayment({ orderId, paymentId, signature, transactionId }) {
  ...
  // Log the action
  await logPaymentAction(transaction.createdBy, "verify", transaction.id, {
    paymentId,
  });
...
async function processRefund(refundData, processedBy) {
  ...
  try {
    ...
    // Log the action
    await logPaymentAction(processedBy, "refund", transactionId, {
      amount: refundAmount,
      reason,
    });
...
async function getPaymentSummary(companyId, { startDate, endDate }) {
  ...
  const [totalRevenue, transactionsByStatus, transactionsByGateway] =
    await Promise.all([
