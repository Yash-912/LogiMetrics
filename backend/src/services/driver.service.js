...
const { Driver, Vehicle, Shipment, Company, User, DriverDocument } = require('../models/postgres');
 ...
 */
async function getDrivers({ page = 1, limit = 10, search, status, companyId, licenseType, sortBy = 'createdAt', sortOrder = 'desc' }) {
    ...
    const { count, rows } = await Driver.findAndCountAll({
        where,
        include: [
            { model: Vehicle, as: 'currentVehicle', attributes: ['id', 'registrationNumber', 'type'] }
        ],
...
async function createDriver(driverData, createdBy) {
    const {
        companyId, firstName, lastName, email, phone, dateOfBirth,
        licenseNumber, licenseType, licenseExpiry, address, city, state, country, postalCode,
        emergencyContact, emergencyPhone, employmentType, joiningDate
    } = driverData;
...
async function updateDriver(driverId, updateData, updatedBy) {
    ...
    // Log the action
    await logDriverAction(updatedBy, 'update', driverId, { fields: Object.keys(updateData) });
...
async function updateDriverStatus(driverId, status, notes, updatedBy) {
    ...
    // Log the action
    await logDriverAction(updatedBy, 'status_change', driverId, { status, notes });
...
async function getPerformance(driverId, { startDate, endDate }) {
    ...
    const totalShipments = shipments.reduce((sum, s) => sum + parseInt(s.dataValues.count), 0);
    const deliveredShipments = shipments.find(s => s.status === 'delivered')?.dataValues.count || 0;
    const failedDeliveries = shipments.find(s => s.status === 'failed_delivery')?.dataValues.count || 0;
    ...
    return {
        ...
        onTimeDeliveryRate: deliveredShipments > 0 ? ((onTimeDeliveries / deliveredShipments) * 100).toFixed(2) : 0,
