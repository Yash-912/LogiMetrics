...
const { Company, User, Subscription, CompanySetting } = require('../models/postgres');
 ...
 */
async function getCompanies({ page = 1, limit = 10, search, status, industry, sortBy = 'createdAt', sortOrder = 'desc' }) {
...
async function createCompany(companyData, createdBy) {
    const { name, email, phone, address, city, state, country, postalCode, industry, website, taxId } = companyData;
...
async function updateCompany(companyId, updateData, updatedBy) {
    ...
    // Log the action
    await logCompanyAction(updatedBy, 'update', companyId, { fields: Object.keys(updateData) });
...
async function deleteCompany(companyId, deletedBy) {
    ...
    if (activeUsers > 0) {
        throw new Error('Cannot delete company with active users. Please deactivate all users first.');
    }
...
async function updateCompanyStatus(companyId, status, reason, updatedBy) {
    ...
    for (const admin of admins) {
        try {
            await sendEmail({
                ...
                html: `<p>Your company status has been changed to: <strong>${status}</strong></p>${reason ? `<p>Reason: ${reason}</p>` : ''}`
            });
        } catch (error) {
    ...
    // Log the action
    await logCompanyAction(updatedBy, 'status_change', companyId, { status, reason });
...
async function updateCompanySettings(companyId, settingsData, updatedBy) {
    ...
    // Log the action
    await logCompanyAction(updatedBy, 'settings_update', companyId, { fields: Object.keys(settingsData) });
...
async function updateSubscription(companyId, subscriptionData, updatedBy) {
    ...
    // Log the action
    await logCompanyAction(updatedBy, 'subscription_update', companyId, { plan, billingCycle });
...
async function cancelSubscription(companyId, reason, cancelledBy) {
    ...
    // Log the action
    await logCompanyAction(cancelledBy, 'subscription_cancel', companyId, { reason });
 ...
 */
async function getTeamMembers(companyId, { page = 1, limit = 20, role, status }) {
    const offset = (page - 1) * limit;
 ...
 * Invite team member
 */
async function inviteTeamMember(companyId, { email, role, firstName, lastName }, invitedBy) {
    ...
    // Log the action
    await logCompanyAction(invitedBy, 'invite_member', companyId, { email, role });
