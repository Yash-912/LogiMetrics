...
async function getDashboardStats(companyId, dateRange = {}) {
  ...
  const [
    ...
    Shipment.count({
      where: {
        ...where,
        status: { [Op.in]: ["pending", "in_transit", "out_for_delivery"] },
      },
    }),
...
async function getShipmentAnalytics(companyId, startDate, endDate) {
  ...
  const [byStatus, byDay, deliveryTimes, onTimeDeliveries, totalDelivered] =
    await Promise.all([
      Shipment.findAll({
        where,
        attributes: [
      ...
      Shipment.count({
        where: {
          ...where,
          status: "delivered",
          deliveredAt: { [Op.lte]: Sequelize.col("estimatedDelivery") },
        },
      }),
      Shipment.count({ where: { ...where, status: "delivered" } }),
    ]);
  ...
  return {
    byStatus,
    byDay,
    avgDeliveryTime: deliveryTimes[0]?.avgHours
      ? parseFloat(deliveryTimes[0].avgHours).toFixed(1)
      : 0,
    onTimeDeliveryRate: totalDelivered
      ? ((onTimeDeliveries / totalDelivered) * 100).toFixed(1)
      : 0,
  };
}
...
async function getRevenueAnalytics(companyId, startDate, endDate) {
  ...
  const [byDay, byMonth, totalRevenue, totalRefunds, avgTransaction] =
    await Promise.all([
      Transaction.findAll({
        where,
        attributes: [
  ...
  return {
    ...
    netRevenue: (totalRevenue || 0) - Math.abs(totalRefunds || 0),
    avgTransactionValue: avgTransaction?.avg
      ? parseFloat(avgTransaction.avg).toFixed(2)
      : 0,
  };
}
...
async function calculateKPIs(companyId, period = "month") {
  ...
  const startDate = new Date(
    now.getTime() - (periodDays[period] || 30) * 24 * 60 * 60 * 1000
  );
