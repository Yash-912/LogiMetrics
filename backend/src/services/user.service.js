...
const { User, Company, Role, Permission } = require('../models/postgres');
 ...
 */
async function getUsers({ page = 1, limit = 10, search, status, role, companyId, sortBy = 'createdAt', sortOrder = 'desc' }) {
...
async function createUser(userData, createdBy) {
    const { email, password, firstName, lastName, companyId, role = 'user', phone, avatar } = userData;
...
async function updateUser(userId, updateData, updatedBy) {
    ...
    // Log the action
    await logUserAction(updatedBy, 'update', userId, { fields: Object.keys(safeUpdateData) });
...
async function updateProfile(userId, profileData) {
    ...
    const allowedFields = ['firstName', 'lastName', 'phone', 'timezone', 'language', 'bio', 'address'];
    const updateData = {};
...
function sanitizeUser(user) {
    ...
    const { password, passwordResetToken, emailVerificationToken, ...sanitized } = userData;
    return sanitized;
}
