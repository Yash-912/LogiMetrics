...
const { Route, Waypoint, Shipment, Driver, Vehicle, Company } = require('../models/postgres');
const { AuditLog } = require('../models/mongodb');
const { getDirections, getDistanceMatrix, getMapboxRoute, calculateETA } = require('../config/maps');
 ...
 */
async function getRoutes({ page = 1, limit = 10, search, status, companyId, driverId, startDate, endDate, sortBy = 'createdAt', sortOrder = 'desc' }) {
    ...
    const { count, rows } = await Route.findAndCountAll({
        ...
        include: [
            ...
            { model: Vehicle, as: 'vehicle', attributes: ['id', 'registrationNumber'] },
...
async function createRoute(routeData, createdBy) {
    const {
        companyId, name, description, startLocation, endLocation,
        waypoints, driverId, vehicleId, scheduledStartTime, scheduledEndTime,
        type, priority
    } = routeData;
...
async function updateRoute(routeId, updateData, updatedBy) {
    ...
    // Log the action
    await logRouteAction(updatedBy, 'update', routeId, { fields: Object.keys(updateData) });
...
async function addWaypoint(routeId, waypointData, addedBy) {
    ...
    const maxSequence = route.waypoints.reduce((max, wp) => Math.max(max, wp.sequence), 0);
    ...
    // Log the action
    await logRouteAction(addedBy, 'add_waypoint', routeId, { waypointId: waypoint.id });
...
async function optimizeRoute(routeId, options, optimizedBy) {
    ...
    try {
        ...
        const waypointCoords = route.waypoints.map(wp => `${wp.latitude},${wp.longitude}`);
        const directions = await getDirections(
...
async function cloneRoute(routeId, newName, clonedBy) {
    ...
    // Log the action
    await logRouteAction(clonedBy, 'clone', newRoute.id, { sourceRouteId: routeId });
