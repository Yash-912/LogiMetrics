...
async function updateShipment(shipmentId, updateData, updatedBy) {
  ...
  // Log the action
  await logShipmentAction(updatedBy, "update", shipmentId, {
...
async function updateShipmentStatus(
  ...
  // Log the action
  await logShipmentAction(updatedBy, "status_change", shipmentId, {
    oldStatus,
    newStatus,
  });
...
async function assignShipment(shipmentId, { driverId, vehicleId }, assignedBy) {
  ...
  await addShipmentEvent(
    shipment.id,
    "assigned",
    `Assigned to driver and vehicle`,
    assignedBy,
    { driverId, vehicleId }
  );
...
async function uploadPOD(
  shipmentId,
  { signature, photo, notes, recipientName },
  uploadedBy
) {
  const shipment = await Shipment.findByPk(shipmentId);
  ...
  if (signature) {
    podData.signatureUrl = await uploadToS3(
      signature,
      `shipments/${shipmentId}/pod/signature`
    );
  }

  // Upload photo if provided
  if (photo) {
    podData.photoUrl = await uploadToS3(
      photo,
      `shipments/${shipmentId}/pod/photo`
    );
  }
...
async function getShipmentEvents(shipmentId) {
  const events = await ShipmentEvent.find({ shipmentId }).sort({
    timestamp: -1,
  });
...
async function getShipmentStats(companyId, { startDate, endDate }) {
  ...
  const delivered =
    stats.find((s) => s.status === "delivered")?.dataValues.count || 0;
...
async function notifyShipmentCreated(shipment) {
  try {
    ...
    logger.info(
      `Shipment created notification sent for ${shipment.trackingId}`
    );
  } catch (error) {
...
async function notifyStatusChange(shipment, newStatus) {
  try {
    ...
    logger.info(
      `Status update notification sent for ${shipment.trackingId}: ${newStatus}`
    );
  } catch (error) {
