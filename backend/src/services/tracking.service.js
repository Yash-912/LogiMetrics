...
async function getShipmentLocationHistory(shipmentId, { startDate, endDate }) {
  ...
  const history = await LiveTracking.find(query).sort({ timestamp: 1 });
 ...
 */
async function triggerGeofenceAlert(
  vehicleId,
  shipmentId,
  geofence,
  eventType
  ...
  logger.info(
    `Geofence ${eventType}: vehicle ${vehicleId}, geofence ${geofence.name}`
  );
}
...
async function checkTelemetryAlerts(vehicleId, telemetry) {
  ...
  if (
    telemetry.engineTemperature !== null &&
