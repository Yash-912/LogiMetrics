...
export default function LeafletHeatmap() {
  ...
  // LOCAL SIMULATION with realistic routes
  const startLocalSimulation = () => {
    ...
    // Create routes that go through accident zones
    if (accidentZones.length > 0) {
      // Get first 3 accident zones as destinations
      const targets = accidentZones.slice(0, 3);

      trucksRef.current = targets.map((zone, idx) => {
        const [zoneLng, zoneLat] = zone.location.coordinates;
        // Start 5km away from the zone
      ...
      // Fallback routes if no zones loaded
      trucksRef.current = [
        {
          id: "TRUCK-001",
          lat: 18.47,
    ...
    simulationIntervalRef.current = setInterval(() => {
      trucksRef.current.forEach((truck) => {
        ...
        // Check for accident zone proximity
  ...
  useEffect(() => {
    ...
    loadScripts().then((L) => {
      ...
      // Load accident zones from backend
      fetch("http://localhost:3000/api/v1/accidents/heatmap")
        ...
        .catch((err) => {
          addLog("❌ Failed to load zones - backend offline");
          setIsLoading(false);
        });
  ...
  return (
    <div className="w-full h-screen flex flex-col bg-gray-900">
      ...
      <div className="bg-gray-800 border-b border-gray-700 p-3 flex gap-3 items-center">
        ...
        >
          {useLocalSimulation ? "⛔ Stop" : "▶️ Start"} Simulation
        </button>
